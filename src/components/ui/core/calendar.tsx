...
function CalendarMonthCaption({
  ...
  className,
  ...captionProps
}: React.ComponentProps<NonNullable<DayPickerComponents["MonthCaption"]>>) {
  ...
  const minYear = propStartMonth
    ? propStartMonth.getFullYear()
    : baseYear - 100;

  const maxYear = propEndMonth
    ? propEndMonth.getFullYear()
    : baseYear + 100;
...
function CalendarWeeks({
  ...
  children,
  ...weekProps
}: React.ComponentProps<NonNullable<DayPickerComponents["Weeks"]>>) {
...
function CalendarMonthComponent({
  ...
  className,
  ...monthProps
}: React.ComponentProps<NonNullable<DayPickerComponents["Month"]>>) {
...
const defaultComponents: DayPickerComponents = {
...
function Calendar({
  ...
  const [direction, setDirection] = React.useState<NavigationDirection>(null);
  const [baseYear] = React.useState<number>(() => new Date().getFullYear());
